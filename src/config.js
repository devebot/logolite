...
function Configuration () {
  ...
  let properties = {
    DEFAULT_INSTANCE_ID: {
      get: function() {
        ...
        store.DEFAULT_INSTANCE_ID = store.DEFAULT_INSTANCE_ID ||
            getEnvOpt('LOGOLITE_INSTANCE_ID') || self.getLogID();
    ...
    PREFIX_OF_EXTRA: {
      get: function() {
        isLiveLoad && doLiveLoad('PREFIX_OF_EXTRA');
        if (!store.PREFIX_OF_EXTRA) {
          store.PREFIX_OF_EXTRA = getEnvOpt('LOGOLITE_PREFIX_OF_EXTRA') || '-E-';
          store.PREFIX_OF_EXTRA = ' ' + store.PREFIX_OF_EXTRA + ' ';
        }
        return store.PREFIX_OF_EXTRA;
      }
    },
    PREFIX_OF_TAGS: {
      get: function() {
  ...
  this.sortLevels = function(levelMap, isInteger) {
  ...
  this.isAlwaysEnabledFor = function(level) {
    if (this.ALWAYS_ENABLED.indexOf('all') >= 0) return true;
    return this.ALWAYS_ENABLED.indexOf(level) >= 0;
  }

  this.isAlwaysMutedFor = function(level) {
    if (this.ALWAYS_MUTED.indexOf('all') >= 0) return true;
    return this.ALWAYS_MUTED.indexOf(level) >= 0;
  }

  this.getLogID = function(opts) {
    if ((opts && opts.uuid) || !this.USE_BASE64_UUID) return uuidV4();
  ...
  Object.defineProperties(this, {
    libraryInfo: {
      get: function() {
        if (libraryInfo == null) {
          let pkgInfo = self.getPackageInfo();
          libraryInfo = {
