...
var LogTracer = function(params) {
  ...
  self.toMessage = function(opts) {
    ...
    if (LogConfig.IS_TEXT_EMBEDDABLE || LogConfig.IS_TEXTFORMAT_ENABLED) {
    ...
    if (LogConfig.IS_STRINGIFY_ENABLED || opts.stringify) {
      if (LogConfig.IS_TEXTFORMAT_ENABLED && text) {
        output = text;
      } else {
      ...
      if (LogConfig.IS_TEXT_EMBEDDABLE && text) {
        output[LogConfig.TEXT_FIELD_NAME]= text;
      }
    }
    if (LogConfig.IS_INTERCEPTOR_ENABLED) {
      if (LogConfig.IS_TAGS_EMBEDDABLE && tags) {
    ...
    get: function() {
      return function(accumulator, mappings, logobj, tags) {
        ...
        for(var i=0; i<mappings.length; i++) {
          ...
          var p1 = matchField(logobj, mappings[i].matchingPath, mappings[i].matchingRule);
          var p2 = matchTags(tags, mappings[i].anyTags, mappings[i].allTags);
          var p3 = matchFilter(logobj, mappings[i].filter);
...
var matchField = function(object, fieldPath, matchingRule) {
  var passed = null;
...
var matchFilter = function(packet, filter) {
  var passed = null;
  if (filter instanceof Function) {
    passed = (filter.call(null, packet) == true);
  }
  return passed;
}
