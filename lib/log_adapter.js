...
var Logger = function(kwargs) {
  var self = this;
  var debugLog = debug(kwargs.scope || LogHelper.DEFAULT_SCOPE);

  self.isEnabledFor = function(level) {
    var realLogger = kwargs.store.realLogger;
    if (LogHelper.isAlwaysEnabledFor(level)) return true;
    if (LogHelper.IS_DEBUGLOG || level === 'conlog') return debugLog.enabled;
    return realLogger != null && realLogger[level] !== undefined;
  }

  self.log = function(level) {
    var realLogger = kwargs.store.realLogger;
    if (LogHelper.IS_DEBUGLOG || level === 'conlog') {
...
var detectDefaultLogger = function() {
  switch(LogHelper.AUTO_DETECT_FOR) {
    case 'bunyan':
      try {
...
var LogAdapter = function() {
  ...
  this.connectTo = function(logger) {
    ...
    LX.log('info', LogTracer.ROOT.add(LogHelper.libraryInfo).toMessage({reset: true}));
  }
