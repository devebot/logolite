...
var Logger = function(kwargs) {
  ...
  self.isEnabledFor = function(level) {
    ...
    if (LogConfig.IS_DEBUGLOG || level === LogConfig.DEBUGLOG_NAME) return debugLog.enabled;
    return realLogger != null && realLogger[level] !== undefined;
  }

  self.log = function(level) {
    var realLogger = kwargs.store.realLogger;
    if (LogConfig.IS_DEBUGLOG || level === LogConfig.DEBUGLOG_NAME) {
...
var LogAdapter = function() {
  var store = { realLogger: null, isInfoSent: false };
  ...
  this.connectTo = function(logger) {
    var changed = (logger && (logger !== store.realLogger));
    if (changed) {
      store.realLogger = logger;
    }
    if (changed || !store.isInfoSent) {
      this.getLogger().log('info', LogTracer.ROOT
          .add(LogConfig.libraryInfo).toMessage({reset: true}));
      store.hasInfoSent = true;
    }
  }
